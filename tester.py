...
from tcec_sampling.tcec_sampling import TcecSampler
...
def tester_theoretical_criterion_sampling(verbose=False):
    ...
    sampler = TcecSampler()
    sampler.sample(first_node=np.random.choice(list(G.nodes())), directed=G.is_directed(), successors=successors,
                   sample_size=N_nodes_s, count_type='nodes', patience=1000, neigh_eval_frac=0.1, verbose=verbose)
    subG_n = sampler.subG
    ...
    sampler = TcecSampler()
    sampler.sample(first_node=np.random.choice(list(G.nodes())), directed=G.is_directed(), successors=successors,
                   sample_size=N_nodes_s, count_type='edges', patience=1000, neigh_eval_frac=0.1, verbose=verbose)
    subG_e = sampler.subG
    ...
    print('\nSampling with minimum requirment on nodes. Minimum number:', N_nodes_s)
    sampler = TcecSampler()
    sampler.sample(first_node=np.random.choice(list(G.nodes())), directed=G.is_directed(), patience=1000,
                   predecessors=predecessors, successors=successors, sample_size=N_nodes_s, count_type='nodes',
                   neigh_eval_frac=0.1, verbose=verbose)
    subG_n = sampler.subG
    print('Sampled graph. Nodes and edges:', subG_n.number_of_nodes(), subG_n.number_of_edges())
    ...
    sampler = TcecSampler()
    sampler.sample(first_node=np.random.choice(list(G.nodes())), directed=G.is_directed(), patience=1000,
                   predecessors=predecessors, successors=successors, sample_size=N_nodes_s, count_type='edges',
                   neigh_eval_frac=0.1, verbose=verbose)
    subG_e = sampler.subG
    print('Sampled graph. Nodes and edges:', subG_e.number_of_nodes(), subG_e.number_of_edges())
